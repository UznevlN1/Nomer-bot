...
COUNTRY_NAMES = {
    ...
    "JP": "Yaponiya", "KR": "Janubiy Koreya", "BD": "Bangladesh",
    ...
    "PS": "Falastin", "MM": "Myanma (Birma)", "ZW": "Zimbabve",
    ...
    "LA": "Laos", "TW": "Tayvan", "ZA": "Janubiy Afrika",
    "MZ": "Mozambik", "AO": "Angola", "CM": "Kamerun",
    "SN": "Senegal", "CI": "Kot-d'Ivuar", "ML": "Mali",
    "BF": "Burkina-Faso", "TD": "Chad", "NE": "Niger",
    "SO": "Somali", "LR": "Liberiya", "SL": "Syerra-Leone",
    "TG": "Togo", "CF": "Markaziy Afrika",
    "CG": "Kongo", "CD": "Kongo (DRK)", "RW": "Ruanda",
    "BI": "Burundi", "MW": "Malavi", "ZM": "Zambiya",
    "BW": "Botsvana", "NA": "Namibiya", "LS": "Lesoto",
    "SZ": "Svazilend", "MG": "Madagaskar", "MU": "Mavrikiy",
    "DJ": "Jibuti", "ER": "Eritreya", "SS": "Jan. Sudan",
    "MR": "Mavritaniya", "GN": "Gvineya", "GW": "Gvineya-Bisau",
    "GM": "Gambiya", "CV": "Kabo-Verde", "GQ": "Ekv. Gvineya",
    "GA": "Gabon", "BJ": "Benin",
    "IE": "Irlandiya", "GD": "Grenada", "VC": "Sent-Vinsent",
    "GU": "Guam", "AT": "Avstriya", "CR": "Kosta-Rika",
    "PA": "Panama", "GT": "Gvatemala", "HN": "Gonduras",
    "SV": "Salvador", "NI": "Nikaragua", "CU": "Kuba",
    "DO": "Dominikan", "HT": "Gaiti", "JM": "Yamayka",
    "TT": "Trinidad va Tobago", "BB": "Barbados", "LC": "Sent-Lyusiya",
    "AG": "Antigua va Barbuda", "DM": "Dominika", "KN": "Sent-Kits",
    "BS": "Bagama orollari", "TC": "Turks va Kaykos", "BM": "Bermuda",
    "KY": "Kayman orollari", "VG": "Britaniya Virgin orollari",
    "VI": "AQSh Virgin orollari", "PR": "Puerto-Riko",
    "CH": "Shveytsariya", "LU": "Lyuksemburg", "MT": "Malta",
    "CY": "Kipr", "IS": "Islandiya", "AL": "Albaniya",
    "MK": "Shimoliy Makedoniya", "RS": "Serbiya", "ME": "Chernogoriya",
    "BA": "Bosniya va Gersegovina", "HR": "Xorvatiya", "SI": "Sloveniya",
    "BG": "Bolgariya", "LI": "Lixtenshteyn", "AD": "Andorra",
    "SM": "San-Marino", "MV": "Maldiv orollari", "BT": "Butan",
    "FJ": "Fiji", "PG": "Papua Yangi Gvineya", "SB": "Solomon orollari",
    "VU": "Vanuatu", "WS": "Samoa", "TO": "Tonga", "KI": "Kiribati",
    "FM": "Mikroneziya", "PW": "Palau", "NR": "Nauru", "TV": "Tuvalu",
    "TL": "Sharqiy Timor", "BN": "Bruney", "MO": "Makao", "HK": "Gonkong",
}
...
def main_menu(user_id):
    ...
    kb.append([InlineKeyboardButton(text="🏆 Reyting", callback_data="rating")])
    if user_id == ADMIN_ID:
...
async def show_countries(call: CallbackQuery, page=0):
    try:
        ...
        kb.append([
            ...
            InlineKeyboardButton(text="🔍 Qidirish", callback_data="search_country")
        ])
...
async def confirm_buy(call: CallbackQuery):
    ...
    try:
        ...
        msg = await call.message.edit_text(
            ...
            f"💵 Narxi: <b>{price:,} so'm</b>\n\n"
            f"⬇️ Telegram ga kod jo'nating, keyin quyidagi tugmani bosing:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📩 Kodni olish", callback_data=f"get_code_{order_id}")],
        ...
        flag = all_flags.get(country_code, "🌍")
        proof_channel_id = get_setting("proof_channel_id") or "-1002825947105"
        if proof_channel_id:
            try:
                proof_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="👤 Raqam Egasi", url=f"tg://user?id={user_id}")],
                    [InlineKeyboardButton(text="🚀 Bizni botimiz", url="https://t.me/nomerx_uzbot")]
                ])
                await bot.send_message(
                    proof_channel_id,
                ...
                logging.error(f"Proof channel error: {ex}")
        
        # No auto polling - user will press button
...
async def get_code_cb(call: CallbackQuery):
    order_id = call.data.replace("get_code_", "")
    await call.answer("⏳ Kod tekshirilmoqda...")
    
    try:
        data = await pixsim_request("getCode", order_id=order_id)
        if not data.get("ok"):
            await call.message.edit_text(
                "❌ Kod hali kelmadi. Telegram ga kod jo'nating va qayta urining.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="📩 Kodni olish", callback_data=f"get_code_{order_id}")],
                    [InlineKeyboardButton(text="🔙 Menyu", callback_data="main_menu")]
                ])
...
async def pay_approve(call: CallbackQuery):
    ...
    # Admin commission
    commission_percent = int(get_setting("admin_commission") or 10)
    commission = int(payment['amount'] * commission_percent / 100)
    current_earnings = int(get_setting("admin_earnings") or 0)
    set_setting("admin_earnings", str(current_earnings + commission))
...
async def admin_panel(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💳 PIX SIM Balans", callback_data="admin_pixsim_balance")],
        [InlineKeyboardButton(text="🏆 Reyting boshqaruv", callback_data="admin_rating")],
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data="main_menu")]
    ])
...
@dp.callback_query(F.data == "admin_pixsim_balance")
async def admin_pixsim_balance(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        return
    await call.answer("⏳ Tekshirilmoqda...")
    try:
        data = await pixsim_request("getBalance")
        if data.get("ok"):
            balance = data.get("balance", 0)
            currency = data.get("currency", "UZS")
            await call.message.edit_text(
                f"💳 <b>PIX SIM Balans</b>\n\n"
                f"💰 Balans: <b>{int(balance):,} {currency}</b>",
...
async def admin_stats(call: CallbackQuery):
    ...
    admin_earnings = int(get_setting("admin_earnings") or 0)
    commission = int(get_setting("admin_commission") or 10)
    await call.message.edit_text(
        f"📊 <b>Statistika</b>\n\n"
        ...
        f"💎 Admin komissiya ({commission}%): <b>{admin_earnings:,} so'm</b>",
        reply_markup=back_btn("admin_panel"),
        parse_mode="HTML"
    )
...
async def admin_settings(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💎 Admin komissiya", callback_data="set_commission")],
...
async def set_card_owner_cb(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminState.set_card_owner)
    await call.message.edit_text("👤 Karta egasining ismini kiriting:", reply_markup=back_btn("admin_settings"))

@dp.message(AdminState.set_card_owner)
...
async def set_commission_cb(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        return
    await state.set_state(AdminState.set_referral_percent)
    await call.message.answer("💎 Admin komissiya foizi (%) kiriting:")
...
async def rating_cb(call: CallbackQuery):
    top = get_top_buyers(5)
    days_left = get_rating_days_left()
    
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    
    if not top:
        text = "🏆 <b>Reyting</b>\n\nHali hech kim nomer olmagan!"
    else:
        text = f"🏆 <b>Reyting</b>\n\n"
        for i, row in enumerate(top):
            ...
            text += f"{medal} <b>{name}</b> — {count} ta nomer\n"
        text += f"\n📅 Yangilanishga: <b>{days_left} kun</b> qoldi"
        text += f"\n🎁 1-o'rin sovg'a oladi!"
...
async def admin_rating_cb(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        return
    ...
    if not top:
        ...
        text = f"🏆 <b>Reyting boshqaruv</b>\n\n"
        for i, row in enumerate(top):
            name = row['full_name'] or "Noma'lum"
            username = f"@{row['username']}" if row['username'] else f"ID: {row['user_id']}"
            count = row['order_count']
            medal = medals[i] if i < len(medals) else f"{i+1}."
            text += f"{medal} <b>{name}</b> ({username}) — {count} ta\n"
        text += f"\n📅 Yangilanishga: <b>{days_left} kun</b> qoldi"
        
        winner_id = top[0]['user_id']
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎁 G'olibga xabar yuborish", callback_data=f"send_gift_{winner_id}")],
            [InlineKeyboardButton(text="🔄 Reytingni yangilash", callback_data="reset_rating")],
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="admin_panel")]
        ])
...
async def send_gift_cb(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        return
    winner_id = int(call.data.split("_")[-1])
    await state.update_data(winner_id=winner_id)
    await state.set_state(RatingState.gift_message)
    await call.message.answer("🎁 G'olibga yubormoqchi bo'lgan xabaringizni yozing:")

@dp.message(RatingState.gift_message)
async def gift_message_send(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    data = await state.get_data()
    ...
    try:
        await bot.send_message(
            winner_id,
            f"🎉 <b>Tabriklaymiz!</b>\n\n"
            f"Siz bu oy eng ko'p nomer olgan foydalanuvchi sifatida\n"
            f"<b>🎁 Sovg'aga</b> loyiq bo'ldingiz!\n\n"
            f"Admin xabari: {message.text}",
            parse_mode="HTML"
        )
        await message.answer("✅ G'olibga xabar yuborildi!")
    except:
        await message.answer("❌ Xabar yuborib bo'lmadi!")
    await state.clear()

@dp.callback_query(F.data == "reset_rating")
async def reset_rating_cb(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        return
    reset_rating()
    await call.answer("✅ Reyting yangilandi! 30 kunlik hisob boshlandi.")
    await call.message.edit_text(
        "✅ Reyting yangilandi!\n📅 Yangi 30 kunlik davr boshlandi.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏆 Reytingga qaytish", callback_data="admin_rating")],
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="admin_panel")]
        ])
    )
