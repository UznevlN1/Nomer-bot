...
def reset_rating():
    ...
    if row:
        ...
        conn.execute("INSERT INTO settings (key, value) VALUES ('rating_reset_date', ?)",
                     (datetime.datetime.now().strftime('%Y-%m-%d'),))
    conn.commit()
    conn.close()
...
def get_rating_days_left():
    ...
    if not row:
        ...
        conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('rating_reset_date', ?)", (today,))
        conn.commit()
        conn.close()
        return 30
    conn.close()
